...
def __W_DBT_RH_P(DBT, RH, P):
    print(f"outputs: {DBT}, {RH}, {P}")
    assert __valid_DBT(DBT) == True
    Pw=RH*__Pws(DBT)
    #print(f"output: {0.621945*Pw/(P-Pw)}")
...
def __W_DBT_WBT_P(DBT, WBT, P):
    if __valid_DBT(DBT):
        DBT=DBT-273.15
        WBT=WBT-273.15
        #print(f"term a: {(2501-2.326*WBT)}")
        print(f"term b: {__W_DBT_RH_P(WBT+273.15,1,P)}")
